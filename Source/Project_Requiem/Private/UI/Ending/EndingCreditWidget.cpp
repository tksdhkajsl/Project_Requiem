...
void UEndingCreditWidget::NativeTick(const FGeometry& MyGeometry, float DeltaTime)
{
    ...
    float EndOffset = CreditScrollBox->GetScrollOffsetOfEnd();
    float DistanceToEnd = EndOffset - CurrentOffset;

    float Speed = ScrollSpeed;
    if (DistanceToEnd < 300.f) {
        float SpeedScale = FMath::Clamp(DistanceToEnd / 300.f, 0.5f, 1.0f);
        Speed *= SpeedScale;
    }

    float NewOffset = CurrentOffset + Speed * DeltaTime;
    CreditScrollBox->SetScrollOffset(NewOffset);

    if (NewOffset >= EndOffset) EndCredit();
}

void UEndingCreditWidget::EndCredit()
{
