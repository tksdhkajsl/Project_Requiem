...
void UPRHUDWidget::BindToPawn(APawn* NewPawn)
{
    if (UStatComponent* StatComp = NewPawn->FindComponentByClass<UStatComponent>()) {
        ...
        // 스탯 레벨용 갱신 (최대 체력/ 스태미나 증가시 갱신)
        StatComp->OnLevelUpStatsUpdated.RemoveDynamic(this, &UPRHUDWidget::HandleLevelUpStatsUpdated);
        StatComp->OnLevelUpStatsUpdated.AddDynamic(this, &UPRHUDWidget::HandleLevelUpStatsUpdated);
    }
}
...
void UPRHUDWidget::HandleLevelUpStatsUpdated(ELevelUpStats StatType, int32 AllocatedPoints)
{
    if (Player && Player->GetStatComponent())
    {
        UStatComponent* StatComp = Player->GetStatComponent();

        // 1. 체력바 강제 갱신 (힘을 찍어서 MaxHP가 늘었을 수 있음)
        float CurHP = StatComp->GetStatCurrent(EFullStats::Health);
        float MaxHP = StatComp->GetStatMax(EFullStats::Health);
        UpdateHPBar(CurHP, MaxHP);

        // 2. 스태미나바 강제 갱신 (민첩을 찍어서 MaxST가 늘었을 수 있음)
        float CurST = StatComp->GetStatCurrent(EFullStats::Stamina);
        float MaxST = StatComp->GetStatMax(EFullStats::Stamina);
        UpdateSTBar(CurST, MaxST);
    }
